...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLogin, companyName }) => {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col items-center justify-center bg-slate-950 p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-sm bg-slate-900/80 backdrop-blur-md border border-slate-700 rounded-2xl p-8 shadow-2xl relative z-10 flex flex-col">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white tracking-tight">{companyName.toUpperCase()} CORP</h1>
          <p className="text-slate-400 text-sm">Portal de Comunicação</p>
          <p className="text-cyan-500/80 text-[11px] italic mt-2 font-medium">"O sistema que cuida do sistema"</p>
        </div>
      ...
      <div className="absolute bottom-4 left-0 w-full text-center pointer-events-none">
         <p className="text-[10px] text-slate-600 font-mono tracking-wider">
            TECHNOLOGY BY <span className="font-bold text-slate-500">NEXUS CORP</span>
         </p>
      </div>
    </div>
  );
